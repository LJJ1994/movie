...
import HotEvaluation from './component/HotEvaluation'
export default {
  name: 'MovieDetail',
  data() {
    return {
  ...
  created() {
    ...
    this.requestData(`/api/cc_info.json`, (response) => {
      let data = response.data
      this.infoObj = data.data.returnValue
      this.initSwiper()
    })
    this.requestData(`/api/cc_evaluation.json`, (response) => {
      let data = response.data
      this.evalLists = data.data.returnValue
      this.initSwiper()
    })
  }
